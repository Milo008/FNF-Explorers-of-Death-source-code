...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'den': //Week 1 (EoD)
				var denFront:BGSprite = new BGSprite('denfront', -600, 100, 1.0, 1.0);
				add(denFront);

				var denDoor:BGSprite = new BGSprite('dendoor', -500, 0, 1.1, 1.1);
				denDoor.setGraphicSize(Std.int(denDoor.width * 0.9), Std.int(denDoor.height * 0.9));
				denDoor.updateHitbox();
				add(denDoor);

				case 'cave': //Week 2 (EoD)
				var caveBack:BGSprite = new BGSprite('caveback', -600, -200, 0.9, 0.9);
				add(caveBack);

				var caveFront:BGSprite = new BGSprite('cavefront', -650, -300, 0.9, 0.9);
				caveFront.setGraphicSize(Std.int(caveFront.width * 1.3));
				caveFront.updateHitbox();
				add(caveFront);

				var glitchAnim:FlxSprite = new FlxSprite(-600, -200);
				glitchAnim.frames = Paths.getPackerAtlas('glitch');
				glitchAnim.animation.add('glitch anim', [0, 1, 2, 3], 3);
				glitchAnim.animation.play('glitch anim');
				glitchAnim.scrollFactor.set(0.85, 0.85);
				add(glitchAnim);
				glitchAnim.antialiasing = false;
